...
#Game Control Object      
# <--NOAH--> added setgame method for all rooms added to game. Slightly bad memory-wise, but faster and generally useful. 
class Game:
    ...
    def addRoom(self, r):
        self.rooms.append(r)
        r.setGame(self)
    # <--NOAH--> change room at specific position
    def setRoomAtPos(self, r, pos):
        self.rooms[pos] = r
        r.setGame(self)
    def setRoomAtPosUninitOthers(self, r, pos, emptyRoom):
        for i in range(len(self.rooms)):
            self.rooms[i] = emptyRoom
        self.rooms[pos] = r
        r.setGame(self)
...
class Room:
    ...
    # <--NOAH--> ADDED FOR OVERLAPPING OBJECTS. Now returns an empty list if no found, keep in mind.
    def whatGotClicked(self, findOverlapping = False):
        retArr = None
        if findOverlapping:
            retArr = []
        ...
        if retArr == None:
            ...
            return reversed(retArr)
    def handleClicks(self, findOverlapping = False):
        clicks = self.whatGotClicked(findOverlapping)
        if clicks:
            for obj in clicks:
                cont = obj.click()
                if not cont:
    ...
    def setGame(self, game):
        self._gameThisIsIn = game
    def getGame(self):
        return self._gameThisIsIn
...
# <--NOAH--> added game variable and getter setter for game methods for easy gamewide changes. 
class GameObject(pygame.sprite.Sprite):
    ...
    def setRoom(self, room):
        self._roomThisIsIn= room
    def getRoom(self):
        return self._roomThisIsIn
...
class TextRectangle(GameObject):
    # <--NOAH--> I added centered attributes for x and y
    def __init__(self, text, xPos, yPos, font, textColor, xCentered = False, yCentered = False, buttonWidth = None, buttonHeight = None, buttonColor = None):
        ...
        self.xCentered = xCentered
        self.yCentered = yCentered
        self.buttonWidth = buttonWidth
        self.buttonHeight = buttonHeight
        self.buttonColor = buttonColor
        self.text = text
    ...
    # <--NOAH--> added a buttonifier
    def makeButton(self, buttonWidth, buttonHeight, buttonColor):
        self.buttonWidth = buttonWidth
        self.buttonHeight = buttonHeight
        self.buttonColor = buttonColor
        return TextRectangle(self.text, self.rect.centerx, self.rect.centery, self.font, self.textColor, True, True, self.buttonWidth, self.buttonHeight, self.buttonColor)
